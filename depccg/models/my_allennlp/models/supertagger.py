...
class Supertagger(Model):
    ...
    def forward(self,  # type: ignore
        ...
        if head_indices is not None and head_tags is not None:
            ...
            self._attachment_scores(normalised_arc_logits[:, 1:].contiguous(), head_indices[:, 1:].contiguous(), mask[:, 1:].contiguous())
            self._tagging_accuracy(normalised_head_tag_logits[:, 1:].contiguous(), head_tags[:, 1:].contiguous(), tag_mask[:, 1:].contiguous())
